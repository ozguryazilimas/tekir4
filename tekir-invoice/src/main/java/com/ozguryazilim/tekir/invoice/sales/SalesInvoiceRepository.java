...
import com.ozguryazilim.tekir.entities.SalesInvoice_;
...
@Repository
@Dependent
public abstract class SalesInvoiceRepository extends VoucherRepositoryBase<SalesInvoice, SalesInvoiceViewModel>{
    
    @Override
    public List<SalesInvoiceViewModel> browseQuery(QueryDefinition queryDefinition) {
        List<Filter<SalesInvoice, ?, ?>> filters = queryDefinition.getFilters();

        CriteriaBuilder criteriaBuilder = entityManager().getCriteriaBuilder();
        //Geriye PersonViewModel dönecek cq'yu ona göre oluşturuyoruz.
        CriteriaQuery<SalesInvoiceViewModel> criteriaQuery = criteriaBuilder.createQuery(SalesInvoiceViewModel.class);

        //From 
        Root<SalesInvoice> from = criteriaQuery.from(SalesInvoice.class);
        Join<SalesInvoice, VoucherGroup> joinGroup = from.join(SalesInvoice_.group, JoinType.LEFT);
        ...
        TypedQuery<SalesInvoiceViewModel> typedQuery = entityManager().createQuery(criteriaQuery);
        typedQuery.setMaxResults(queryDefinition.getResultLimit());
        List<SalesInvoiceViewModel> resultList = typedQuery.getResultList();

        return resultList;
    }

    @Override
    public List<SalesInvoice> suggestion(String searchText) {
        throw new UnsupportedOperationException("Not supported yet.");
    }

    private void buildVieModelSelect(CriteriaQuery<SalesInvoiceViewModel> criteriaQuery, Root<? extends SalesInvoice> from) {
        criteriaQuery.multiselect(
                from.get(SalesInvoice_.id),
                ...
                from.get(SalesInvoice_.total),
                from.get(SalesInvoice_.currency)
        );
    }

    private void buildSearchTextControl(String searchText, CriteriaBuilder criteriaBuilder, List<Predicate> predicates, Root<? extends SalesInvoice> from) {
