...
@Repository
@Dependent
public abstract class PurchaseInvoiceRepository extends VoucherRepositoryBase<PurchaseInvoice, PurchaseInvoiceViewModel>{
    
    @Override
    public List<PurchaseInvoiceViewModel> browseQuery(QueryDefinition queryDefinition) {
        List<Filter<PurchaseInvoice, ?, ?>> filters = queryDefinition.getFilters();

        CriteriaBuilder criteriaBuilder = entityManager().getCriteriaBuilder();
        //Geriye PersonViewModel dönecek cq'yu ona göre oluşturuyoruz.
        CriteriaQuery<PurchaseInvoiceViewModel> criteriaQuery = criteriaBuilder.createQuery(PurchaseInvoiceViewModel.class);

        //From 
        Root<PurchaseInvoice> from = criteriaQuery.from(PurchaseInvoice.class);
        Join<PurchaseInvoice, VoucherGroup> joinGroup = from.join(PurchaseInvoice_.group, JoinType.LEFT);
        ...
        TypedQuery<PurchaseInvoiceViewModel> typedQuery = entityManager().createQuery(criteriaQuery);
        typedQuery.setMaxResults(queryDefinition.getResultLimit());
        List<PurchaseInvoiceViewModel> resultList = typedQuery.getResultList();

        return resultList;
    }

    @Override
    public List<PurchaseInvoice> suggestion(String searchText) {
        throw new UnsupportedOperationException("Not supported yet.");
    }

    private void buildVieModelSelect(CriteriaQuery<PurchaseInvoiceViewModel> criteriaQuery, Root<? extends PurchaseInvoice> from) {
        criteriaQuery.multiselect(
                from.get(PurchaseInvoice_.id),
                ...
                from.get(PurchaseInvoice_.total),
                from.get(PurchaseInvoice_.currency)
        );
    }

    private void buildSearchTextControl(String searchText, CriteriaBuilder criteriaBuilder, List<Predicate> predicates, Root<? extends PurchaseInvoice> from) {
