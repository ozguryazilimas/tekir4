...
public class AccountStatementReport extends DynamicReportBase<AccountStatementFilter>{
    ...
    protected void buildReport(JasperReportBuilder report, Boolean forExport) {
        ...
        BigDecimal balance = repository.findByAccountBalance(getFilter().getContact(), getFilter().getBeginDate().getCalculatedValue());
    ...
    protected JRDataSource getReportDataSource() {
        return new JRBeanCollectionDataSource(repository.findByAccountAndDateBetweenOrderByDate(getFilter().getContact(), getFilter().getBeginDate().getCalculatedValue(), getFilter().getEndDate().getCalculatedValue()));
    }
