...
@Repository
@Dependent
public abstract class PurchaseOrderRepository extends VoucherRepositoryBase<PurchaseOrder, PurchaseOrderViewModel>{
    
    @Override
    public List<PurchaseOrderViewModel> browseQuery(QueryDefinition queryDefinition) {
        List<Filter<PurchaseOrder, ?, ?>> filters = queryDefinition.getFilters();

        CriteriaBuilder criteriaBuilder = entityManager().getCriteriaBuilder();
        //Geriye PersonViewModel dönecek cq'yu ona göre oluşturuyoruz.
        CriteriaQuery<PurchaseOrderViewModel> criteriaQuery = criteriaBuilder.createQuery(PurchaseOrderViewModel.class);

        //From 
        Root<PurchaseOrder> from = criteriaQuery.from(PurchaseOrder.class);
        Join<PurchaseOrder, VoucherGroup> joinGroup = from.join(PurchaseOrder_.group, JoinType.LEFT);
        ...
        TypedQuery<PurchaseOrderViewModel> typedQuery = entityManager().createQuery(criteriaQuery);
        typedQuery.setMaxResults(queryDefinition.getResultLimit());
        List<PurchaseOrderViewModel> resultList = typedQuery.getResultList();

        return resultList;
    }

    @Override
    public List<PurchaseOrder> suggestion(String searchText) {
        throw new UnsupportedOperationException("Not supported yet.");
    }

    private void buildVieModelSelect(CriteriaQuery<PurchaseOrderViewModel> criteriaQuery, Root<? extends PurchaseOrder> from) {
        criteriaQuery.multiselect(
                from.get(PurchaseOrder_.id),
                ...
                from.get(PurchaseOrder_.total),
                from.get(PurchaseOrder_.currency)
        );
    }

    private void buildSearchTextControl(String searchText, CriteriaBuilder criteriaBuilder, List<Predicate> predicates, Root<? extends PurchaseOrder> from) {
